...
extension FPUtility{
    ...
    func setNavbarTheme() {
        ...
        // Scroll-edge appearance: keep background consistent to avoid fade/glitch
        let unifiedAppearance = UINavigationBarAppearance()
        if #available(iOS 26.0, *) {
            unifiedAppearance.configureWithTransparentBackground()
            unifiedAppearance.backgroundColor = .systemBackground
        } else if #available(iOS 18.0, *) {
            unifiedAppearance.configureWithOpaqueBackground()
            unifiedAppearance.backgroundEffect = UIBlurEffect(style: .systemMaterial)
            unifiedAppearance.backgroundColor = .systemBackground
        } else {
            unifiedAppearance.configureWithOpaqueBackground()
            unifiedAppearance.backgroundColor = .systemBackground
        ...
        unifiedAppearance.shadowColor = UIColor.black.withAlphaComponent(0.2)
        unifiedAppearance.titleTextAttributes = [
            .font: titleFont,
            .foregroundColor: appTint
        ]
        unifiedAppearance.largeTitleTextAttributes = [
            .font: largeTitleFont,
            .foregroundColor: appTint
        ]
        
        // Use appearance proxies to target labels contained in UINavigationBar
        ...
        // Also allow the back button/title item to scale
        let barButtonLabelAppearance = UILabel.appearance(whenContainedInInstancesOf: [UINavigationBar.self, UIStackView.self])
        barButtonLabelAppearance.adjustsFontSizeToFitWidth = true
        barButtonLabelAppearance.minimumScaleFactor = 0.8
        barButtonLabelAppearance.adjustsFontForContentSizeCategory = true
        
        let navBar = UINavigationBar.appearance()
        navBar.standardAppearance = unifiedAppearance
        navBar.scrollEdgeAppearance = unifiedAppearance
        navBar.compactAppearance = unifiedAppearance
        
        // compactunifiedAppearance can cause glitches on iPadOS 26; set only if available and needed
        if #available(iOS 16.0, *) {
            navBar.compactScrollEdgeAppearance = unifiedAppearance
        }
        
        navBar.tintColor = appTint
        navBar.barStyle = .default
        
        // Let UINavigationBarAppearance manage translucency to avoid layering issues
...
//        navBar.layer.masksToBounds = false
        ...
        // Remove manual layer-based shadows which can conflict with appearance on iOS 26
