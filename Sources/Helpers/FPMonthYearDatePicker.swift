...
open class FPMonthYearDatePicker: UIPickerView {
    ...
    open var mode: DatePickerMode = .monthYear
    ...
    /// - Note: `FPMonthYearDatePicker` does not inherit from `UIControl` so this method is provided only as a way for it be a drop-in replacement for `UIDatePicker` in most scenarios. You can only use the `.valueChanged` control event and you may only set one active target; multiple calls to this method will mean the last call is used as the target / action.
    ...
    /// Stops the delivery of events to the previously set target object.
    ///
    /// - Note: `FPMonthYearDatePicker` does not inherit from `UIControl` so this method is provided only as a way for it be a drop-in replacement for `UIDatePicker` in most scenarios. The parameters used here are meaningless as any call to this method will result in the previously set target / action being removed.
...
extension FPMonthYearDatePicker: UIPickerViewDelegate, UIPickerViewDataSource {
    
    open func numberOfComponents(in pickerView: UIPickerView) -> Int {
        return mode == .monthYear ? 2 : 1
    }
